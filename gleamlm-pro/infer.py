"""GleamLM-Pro 126M 推理脚本。18L×768d，4096 上下文。支持交互式生成和多种采样策略"""
...
def interactive(model, tokenizer, max_new_tokens=512,
                temperature=0.8, top_k=50, top_p=0.9, repetition_penalty=1.1,
                device="cuda", sft_mode=False):
    print("\n" + "=" * 60)
    print("GleamLM-Pro 126M 交互式文本生成")
    if sft_mode:
...
def main():
    parser = argparse.ArgumentParser(description="GleamLM-Pro 126M 推理")
