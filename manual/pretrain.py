...
from torch.nn.parallel import DistributedDataParallel as DDP  # noqa: N817  (framework 约定缩写)
...
def train(args, model_cfg: ModelConfig):
    ...
    if is_main_process():
        ...
        if wandb is not None:
            wandb.init(
                ...
                or f"pretrain_{model_cfg.d_model}d_{model_cfg.num_layers}l",
                config=vars(args),
            )
    ...
    if args.val_data:
        ...
        val_sampler = (
            DistributedSampler(val_dataset, shuffle=False) if dist.is_initialized() else None
        )
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch_idx, batch in batch_iter:
            ...
            sync_ctx = (
                model.no_sync() if (dist.is_initialized() and not is_last_acc) else nullcontext()
            )
            ...
            denom = (
                ((acc_steps % args.accumulate) + 1)
                if batch_idx == len(loader) - 1
                else args.accumulate
            )
...
def _load_training_defaults(training_path: str | None) -> dict[str, Any]:
    ...
    if training_path is None:
        return {}
    cfg = load_config(training_path, scope="training")
    t = cfg.training
    lr = cfg.lr
    return {
        "epochs": t.epochs,
        "batch_size": t.batch_size,
        "accumulate": t.accumulate_grad,
        ...
        "min_lr_ratio": lr.min_lr_ratio,
        "z_loss": cfg.advanced.z_loss_weight,
...
def main():
    ...
    try:
        ...
        if is_main_process():
            print("\n训练被中断 (Ctrl+C)。")
            print(
                f"续训: python manual/pretrain.py --model {pre_args.model} "
