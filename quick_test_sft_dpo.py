"""
烁珑GleamLM SFT+DPO Pipeline 测试 (修复版)
...
from tools.build_dataset import stream_build
...
templates = [
    "今天天气{adj}，适合{act}。", "人工智能是{adj}的技术。",
    "机器学习{adv}改变了我们的{obj}。", "深度学习模型{act}的能力很{adj}。",
    "在{domain}领域，{tech}技术{adv}受欢迎。", "Python是一种{adj}语言。",
]
...
for fname in ["wiki.txt", "qa.txt"]:
    ...
    with open(path, "w", encoding="utf-8") as f:
        for _ in range(3000):
            ...
            line = t.format(adj=random.choice(adj_words), act=random.choice(act_words),
                            adv=random.choice(adv_words), obj=random.choice(obj_words),
                            domain=random.choice(domain_words), tech=random.choice(tech_words))
            f.write(line + "\n")

os.makedirs("data/test_splits", exist_ok=True)
stream_build(
    input_paths=["data/test_raw/wiki.txt", "data/test_raw/qa.txt"],
    output_dir="data/test_splits", train_ratio=0.9, valid_ratio=0.1, buf_size=2000
)

tok = BBPETokenizer.load("./tokenizer/checkpoints/bbpe_12k")
...
sft_prompts = [
    ("你好，请介绍一下你自己。", "你好！我是烁珑GleamLM，一个面向教育和研究的轻量级开源对话模型。"),
...
# 不调用 gleamlm_sft.py（它的 evaluate 会卡死），直接手写最小 SFT 循环
...
# 兼容 gleamlm_dpo.py 读取方式（它也用 model_state_dict key）
...
dpo_data = [
    {"instruction": "你好，请介绍一下你自己。",
     "chosen": "你好！我是烁珑GleamLM，一个面向教育和研究的轻量级开源对话模型。",
...
class DPODataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        chosen_ids = self.tokenizer.encode(item["chosen"] + "<|im_end|>", add_bos=False, add_eos=False)
        rejected_ids = self.tokenizer.encode(item["rejected"] + "<|im_end|>", add_bos=False, add_eos=False)
        # 截断
...
def dpo_collate_fn(batch):
    ...
    for c, r, cm, rm in batch:
        ...
        c_masks.append(torch.cat([cm, torch.zeros(max_c-len(cm), dtype=torch.long)]))
        r_masks.append(torch.cat([rm, torch.zeros(max_r-len(rm), dtype=torch.long)]))
    return (torch.stack(chosen_ids), torch.stack(rejected_ids),
            torch.stack(c_masks), torch.stack(r_masks))
...
for chosen_ids, rejected_ids, c_masks, r_masks in dpo_loader:
    ...
    loss.backward(); dpo_optim.step(); dpo_optim.zero_grad()
    print(f"  DPO loss={loss.item():.4f} | {time.time()-t0:.1f}s")

os.makedirs("checkpoints/dpo_test", exist_ok=True)
torch.save({"model_state_dict": policy.state_dict(), "dpo_loss": loss.item()},
