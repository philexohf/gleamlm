...
def generate_samples(
    ...
    for i, prompt in enumerate(prompts):
        ...
        try:
            ...
            print(
                f"Generated: {result[len(prompt) :].encode('utf-8', errors='replace').decode('utf-8', errors='replace')}"
            )
...
def main():
    ...
    parser.add_argument(
        "--repetition_penalty", type=float, default=1.0, help="重复惩罚（>1 惩罚已出现 token）"
    )
