...
def compute_log_probs(logits, input_ids, mask):
    ...
    log_probs_token = (
        log_probs_all[:, :-1, :].gather(2, input_ids[:, 1:].unsqueeze(-1)).squeeze(-1)
    )  # [B, L-1]
    return (log_probs_token * mask).sum(dim=-1)  # [B]


def dpo_loss(
    policy_chosen_logp, policy_rejected_logp, ref_chosen_logp, ref_rejected_logp, beta=0.1
):
    """DPO 损失"""
...
def train_one_epoch(model, ref_model, dataloader, optimizer, scheduler, scaler, beta, device, args):
    model.train()
    ref_model.eval()
    total_loss = 0.0
    n_batches = 0
