...
|------|------------|------------|------|
| 参数量 | 39M | 0.6B | 15x |
| 训练数据 | 1.2B tokens（4 源混合） | 18T tokens | 15,000x |
| GPU | 1 × 4070 Ti | 数千 GPU·月 | 1,000x+ |
| 人类对齐 | **SFT + DPO 全链路已验证** | SFT + RLHF | 39M 跑通管线；80M 有待产出

### Remark

> 缩小规模差距所需的**工程能力已经具备**——分布式训练（DDP 代码已就绪，`torchrun` 一行启动）、
> 大规模数据过滤、梯度稳定性控制、SFT/DPO 对齐等技术在原理层面已经掌握，只差算力资源。
> XFIND-Nano 的 39M 预训练已收敛至 PPL=34.93（1.2B tokens，4 源混合数据），
> 下一步启动 SFT（指令微调）将续写模型转为对话助手，可选 DPO（偏好对齐）提升安全性和回答质量。
>
> **XFIND-LLM 的定位不是"替代大模型"，而是"证明大模型不再神秘"。**
>
> 已走完 Pretrain → SFT → DPO 全链路，39M 阶段 PPL=34.93，下一步启动 80M 模型预训练。
...
>
> 生成效果：epoch 0 输出模板化循环（"今日天气: 23 点:..."），epoch 1 后能生成完整通顺的中文句子（"一看就是一场狂风暴雨的,你应该看看最近天气情况"），epoch 7 在长 prompt 下可产出主题一致、语法正确的短文。PPL ~35 时采样仍不稳定，短 prompt 容易塌缩——这是 39M 模型的容量天花板。
>
> **SFT 后**：纯文本 Q:/A: 格式可行（当前 tokenizer 不支持 ChatML 特殊 token，将在 80M 阶段修复），995 条蒸馏数据 1 epoch 训练后，模型已学会直接回应问题。示例——机器学习："我要说的是，机器学习不是机器学习的必修课..."；光合作用："光合作用就是指利用光在电磁场中移动的过程..."。内容准确度受 39M 容量限制，但对话格式和话题相关性显著提升。

---
## 开发计划

> 详细的阶段任务、配置方案和数据配比见 **[开发计划文档](docs/XFIND-Lite-语言模型开发计划.md)**（v3.3）。
>
> **当前状态**：V3 预训练完成（4 源混合 1.2B tokens，8 epoch，最优 PPL=34.93），39M 架构已验证完毕。
>
> **P0 — 39M 对齐阶段（已完成**）：
> 1. ✅ 大模型蒸馏生成 SFT 数据 — DeepSeek API 蒸馏，200 种子 → 995 条
> 2. ✅ SFT 指令微调 — 纯文本 Q:/A: + loss mask，1 epoch，best loss=3.72
> 3. ✅ SFT 效果评估 — 模型学会直接回应，话题相关性强
> 4. ✅ DPO 偏好对齐 — 150 对 chosen/rejected，DPO loss 0.95→0.60，流程验证通过
>
> **39M 对齐阶段总结**：SFT 使模型从自由生文转为有方向问答，DPO 流程跑通但质量提升边际（39M 容量天花板）。
> 下一步：启动 80M 预训练基础设施建设（Tokenizer 特殊 token + 5 源数据）。
>
> **P1-P3**：80M 模型训练、消融实验、架构优化（详见开发计划）。
> 13 项预训练质量改进已列入可选规划（涵盖 IMU-1、SmolLM2、Nemotron-Flash 最新技术）。
>
