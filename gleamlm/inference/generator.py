...
def generate_tokens_with_kv(
    ...
    *,
    ...
    amp_dtype: torch.dtype | None = None,
    past_kv: PastKeyValueList | None = None,
) -> tuple[list[int], PastKeyValueList]:
    """Like generate_tokens, but collects all tokens and returns the final KV cache.

    Returns:
        (generated_tokens, final_past_kv) — the final KV cache includes all context
        and generated tokens, ready for multi-turn continuation.
    """
    model.eval()
    tokens: list[int] = []
    generated_ids: list[int] = prompt_ids.copy()
